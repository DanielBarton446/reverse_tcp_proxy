...
static void accept_client(EventSystem* es, TCPServer* server) {
    ...
    int client_fd = accept(server->event.fd, (struct sockaddr*) &client_addr,
                           &client_addr_size);
    if (client_fd == -1) {
...
static void recv_client(EventSystem* es, TCPClient* client) {
    while (1) {
        ssize_t num_bytes = recv(client->event.fd, client->send_buf,
                                 sizeof(client->send_buf), 0);
...
static void echo_client(EventSystem* es, TCPClient* client) {
    while (client->send_offset < client->send_len) {
        ssize_t n =
            send(client->event.fd, client->send_buf + client->send_offset,
